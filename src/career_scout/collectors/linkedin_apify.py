...
class LinkedInApifyCollector:
    """Collect public LinkedIn job postings without using a LinkedIn account.

    Uses an Apify actor backed by LinkedIn's public guest job surfaces. No LinkedIn
    login, cookie or personal-account automation is involved.
    """
    ...
    def search(self, keywords: str, *, where: str = "Sydney, New South Wales, Australia", page: int = 1, sortmode: str = "ListedDate") -> list[Job]:
        ...
        for row in rows:
            ...
            url = self._text(row, "jobUrl", "url", "link")
